...
public abstract class XMLLexer implements Closeable {
	...
	private final void nextTokenBuiler(int expect){
		if(expect==XMLToken.LBRACE
				|| expect==XMLToken.RBRACE
				|| expect==XMLToken.LBRACKET
				|| expect==XMLToken.RBRACKET
    			|| expect==XMLToken.COMMA
    			){
            addbuiler(getCurrent());
    	}
	}
	private final void nextTokenBuiler(){
		if (ch != 't' 
				&& ch != 'T' 
				&& ch != 'S' 
				&& ch != 'f' 
