...
import com.fyk.fastxml.util.Base64;
 ...
 */
public final class XMLScanner extends XMLLexer {

    private final String text;

    public XMLScanner(String input){
        this(input, XML.DEFAULT_PARSER_FEATURE);
    }

    public XMLScanner(String input, int features){
    ...
    public final boolean equalsSymbol(String key){
    	int l=key.length();
    	
    	int t=(getNext() == '"' ? 1 : 0);
    	
    	for (int i = 0; i < l; i++) {
			if( key.charAt(i) != charAt(bp+i) ){
				return false;
			}
		}
    	...
    	return whitespaceFlags[charAt(bp+l)];
    }
    
    public final char getNext(int lenght) {
    	return charAt(bp+lenght);
    }
    

    public XMLScanner(char[] input, int inputLength){
        this(input, inputLength, XML.DEFAULT_PARSER_FEATURE);
    }

    public XMLScanner(char[] input, int inputLength, int features){
    ...
    public boolean scanISO8601DateIfMatch(boolean strict) {
        ...
        if ((!strict) && rest > 13) {
            ...
            if (c0 == '/' && c1 == 'D' && c2 == 'a' && c3 == 't' && c4 == 'e' && c5 == '(' && c_r0 == '/'
                ...
                token = XMLToken.LITERAL_ISO8601_DATE;
                return true;
            }
        }
